...
SPECIFICITY = False
COMBINE_MODELS = True
with open('../.data/test.source') as source, open('test_block_ngram.hypo', 'w') as fout:
    ...
    for sline in source:
        if count % bsz == 0:
            with torch.no_grad():
                if COMBINE_MODELS:
                    hypotheses_batch = bart_beam_decode(bart, [0.3, 0.7], slines, beam_width=3, min_len=3, max_len=20,
                                     max_sentence_count=3, temperature=1, unk_penalty=0.001, block_unigram=3)
                if SPECIFICITY:
