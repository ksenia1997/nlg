...
def convert_gpt_idxs_to_bart(logp, bart_vocab_size, bart_tensor_with_gpt2_idxs):
    ...
    Args:
        logp: word log probabilities
        bart_vocab_size: the size of the BART vocabulary
        bart_tensor_with_gpt2_idxs: a dictionary with GPT2 indexes

    Returns: tensor with converted probabilities to BART vocabulary
...
def generate(model, idf_matrix, tokens: List[torch.LongTensor], beam: int = 5, verbose: bool = False,
    ...
    Args:
        model: BART model
        idf_matrix: a matrix, which contains NIDF metrics for words
        tokens: input tokens
        beam: width of the beam
        verbose: for logging
        **kwargs:
...
class BeamSearchNode(object):
    def __init__(self, previous_node, word_ids, log_prob, length, penalty, skip_ngram_number, prev_context_gpt2,
        ...
        Args:
            previous_node: pointer to the previous BeamSearchNode
            word_ids: indexes of words in the generated sequence
            log_prob: log probabilities of the generated words
            length: the length of the generated sequence
            penalty: penalty for blocking repeated words
            skip_ngram_number: started generation with number of words generated by BART model
            prev_context_gpt2: previous context of GPT-2 model
            max_len: max length for the generated sequence
        """
